...
To plot the outputs created above, first we will draw the Brazilian Biomes on top
...
If you want to save this plot or any other created within R into a file, use functions such as `pdf()`, `jpeg()`, `png()`, `tiff()` before plotting and `dev.off()` afterwards. For example, the following code will create file `amz-cerrado.png` in the current directory to save the plot above.

```{r}
png("amaz-cerrado.png")
