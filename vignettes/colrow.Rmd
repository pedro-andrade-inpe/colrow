...
date: "`r format('24 September 2018')`"
urlcolor: blue
output:
  pdf_document:
    fig_caption: yes
...
This function automatically ignores all attributes that have only one value. The other
attributes (except `ID` and `VALUE`) compose the created attribute names. They are
...
with this data is stored in file `br_biomes.shp` available within `colrow` package.
