...
output:
  ...
  html_document:
    df_print: paged
abstract: This package has functions to handle Simulation Units (SimU), ColRow (CR), and Large Unit (LU) data.
vignette: |
...
The content of the directory should be as shown below.

```{r}
list.files(dataDir)
```

See the countries available. The code below shows only the first 10.

```{r}
colrow::getCountries(dataDir)[1:10]
```

Then use get functions to generate the data for a given country. It must be written
in the same way as one of the outputs of `getCountries()`.
...
These names must have one attribute called `ID` and another called `VALUE`. You can use
`colrow::attrs()` to avoid quotes.
...
It is also possible to save the output when processing a file. In this case, the function
